...
1. **線上展示 / 正式部署模式**：部署於學校 Ubuntu VM，透過 Apache reverse proxy 提供 `/workspace7/` 前端與 `/workspace7/api/` 後端 API，並整合學校 SSO，可直接用瀏覽器開啟https://air.cgu.edu.tw/workspace7/後執行系統，不需額外下載和安裝。
